...
<table id="table" class="sort-table table table-bordered table-hover" data-async="true">
    ...
    <tr>
        ...
        <c:forEach items="${fields}" var="field">
        	<th sort="${field.name}">${field.title}</th>
        </c:forEach>
    </tr>
    </thead>
    <tbody>
    <c:forEach items="${list}" var="map">
        <tr>
            <td class="check"><input type="checkbox" name="ids" value="${map['id']}"></td>
            <td>
                <a class="btn btn-link btn-edit" href="${ctx}/xform/render/${formid}/${map['id']}/update">${map['id']}</a>
            </td>
            <c:forEach items="${fields}" var="field">
            	<td>${map[field.name]}</td>
            </c:forEach>
        </tr>
    </c:forEach>
    </tbody>
