...
<table id="table" class="sort-table table table-bordered table-hover" data-async="true">
    ...
    <c:forEach items="${page.content}" var="m">
        <tr>
            ...
            <td>
                <a href="${ctx}/admin/maintain/extkeyvalue/category/${m.id}">${m.id}</a>
            </td>
            <td>${m.name}</td>
            <td>${m.type.info}</td>
            <td>${m.show ? '是' : '否'}</td>
...
<es:page page="${extKeyValueCategorys}"/>
<script type="text/javascript">
    $(function() {
        $.app.toggleLoadTable($("#table"), "${ctx}/admin/maintain/extkeyvalue/category/{parentId}/keyvalue")
    });
</script>
