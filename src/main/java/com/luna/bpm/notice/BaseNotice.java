...
import com.luna.bpm.persistence.manager.BpmConfNoticeManager;
import com.luna.sys.user.entity.User;
import com.luna.sys.user.repository.UserRepository;

public abstract class BaseNotice {
	private static Logger logger = LoggerFactory.getLogger(BaseNotice.class);
	...
	public void process(DelegateTask delegateTask, int arrivalType) {
		...
		List<BpmConfNotice> bpmConfNotices = bpmConfNoticeManager.find(
...
//		msgConnector.send(subject, content, String.valueOf(user.getId()), null);
	...
	public String getInitiator(UserRepository userRepository,
